...
class GraphVAE(torch.nn.Module):
    def __init__(self, node_feature_dim, state_dim, num_message_passing_rounds, encoder, decoder_net, prior, embedder = None):
        super().__init__()
    ...
    def elbo(self, x):
        ...
        if self.embedder is not None:
            z = self.embedder(z, all_possible_edges.T, x.batch.shape[0])
...
if __name__ == '__main__':
    ...
    gnn_embedder = EmbeddingGNN(node_feature_dim = latent_dim, 
    ...
    model = GraphVAE(node_feature_dim=node_feature_dim, 
                     ...
                     prior = prior, 
                     encoder = encoder, embedder=gnn_embedder).to(device)
